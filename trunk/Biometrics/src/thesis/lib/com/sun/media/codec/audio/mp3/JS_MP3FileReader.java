...
import thesis.lib.sound.sampled.*;
import thesis.lib.sound.sampled.spi.AudioFileReader;
...
public class JS_MP3FileReader extends AudioFileReader
{
    private static class AFF extends AudioFileFormat
    {
        ...
        public AFF(thesis.lib.sound.sampled.AudioFileFormat.Type afft, FrameInfo info)
        {
    ...
    private static class AFFT extends thesis.lib.sound.sampled.AudioFileFormat.Type
    {
...
//        throw exception;
        ...
        throws UnsupportedAudioFileException, IOException
    {
        ...
        int dataOffset = 0;
        thesis.lib.sound.sampled.AudioFileFormat.Type fileType = null;
        ...
        case 1179011410: 
            ...
            isMP3 = dataOffset >= 0;
            maxSyncSearch = 100;
            fileType = thesis.lib.sound.sampled.AudioFileFormat.Type.WAVE;
            break;
    ...
    public static thesis.lib.sound.sampled.AudioFileFormat.Type getFileTypeByInfo(FrameInfo info)
    {
        thesis.lib.sound.sampled.AudioFileFormat.Type type = MPEG;
        if(info.getMpegId() == 1 && info.getLayerId() == 3)
            type = MP3;
        return type;
    }

    public static thesis.lib.sound.sampled.AudioFormat.Encoding getEncByInfo(FrameInfo info)
    {
        if(info.getMpegId() == 1)
    ...
    private static thesis.lib.sound.sampled.AudioFileFormat.Type getType(String name, String ext)
    {
        thesis.lib.sound.sampled.AudioFileFormat.Type ret = null;
        if(!triedTritonus || tGetType != null)
            try
            {
                ...
                args[0] = name;
                args[1] = ext;
                ret = (thesis.lib.sound.sampled.AudioFileFormat.Type)tGetType.invoke(null, args);
                if(DEBUG && firstTime)
    ...
    static final thesis.lib.sound.sampled.AudioFileFormat.Type MPEG = getType("MPEG", "mpeg");
    static final thesis.lib.sound.sampled.AudioFileFormat.Type MP3 = getType("MP3", "mp3");
